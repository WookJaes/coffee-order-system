...
`local` 프로필은 로컬 전용 Flyway V3 migration으로 Postman 검증용 사용자(`userId: 1`)를 등록한다. 공통 migration은 스키마와 메뉴 기준 데이터만 포함하므로 비로컬 환경에는 이 사용자가 새로 생성되지 않는다. 이전에 V3가 적용된 비로컬 DB는 V3를 missing legacy migration으로 허용해 validate/migrate하며, 기존 사용자 row를 삭제하거나 변경하지 않는다.
...
`local` 프로필은 공통 migration(V1/V2)과 로컬 전용 V3를 함께 적용해 Manual HTTP/Postman 예시의 `userId=1`을 준비한다. 비로컬 배포 프로필은 공통 migration만 적용하므로, 배포 자동화나 운영 검증에서 `userId=1`을 전제하지 않는다.
