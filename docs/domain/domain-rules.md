...
- Publisher는 발행 가능한 후보 ID를 batch size까지만 조회한 뒤 `PENDING`·발행 가능 시각 조건의 단일 배치 갱신으로 선점한다. 선점된 이벤트의 Kafka 메시지 필드는 같은 `processing_token`의 DTO projection으로 조회하며, Publisher는 `OrderEvent` Lazy 연관을 순회하지 않는다.
