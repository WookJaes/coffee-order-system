...
- 랭킹 Consumer는 `ranking/consumer`에, Redis 키·Lua 집계 구현은 `ranking/redis`에 둔다. Consumer는 `product-ranking-group`으로 `order-paid`를 소비하고, `resources/scripts/ranking-process-once.lua`를 시작 시 `RedisScript<Long>` Bean으로 로드한다. 이 Lua가 `coffee:ranking:processed:{eventId}` 마커 등록과 `coffee:ranking:{yyyy-MM-dd}`의 `menuId` 점수 1 증가를 원자적으로 처리한다.
