...
- `dataplatform`: Kafka Consumer, 데이터 플랫폼 HTTP 전달
...
- 데이터 플랫폼 Consumer는 `DATA_PLATFORM_CONSUMER_GROUP_ID`로 `order-paid`를 별도 소비한다. `RestClient`의 연결·응답 timeout을 각각 설정하고, HTTP 2xx 뒤에만 listener가 성공 반환한다. 연결 실패·timeout·5xx는 `DATA_PLATFORM_CONSUMER_RETRY_*` 정책을 적용하고, 4xx는 즉시 `DATA_PLATFORM_CONSUMER_DLT_TOPIC`으로 보낸다. DLT send result가 실패하면 recoverer가 실패를 전파해 원본 offset을 완료하지 않는다. 수신 HTTP API는 `Idempotency-Key: order-paid:{eventId}`로 at-least-once 재전송을 논리적 한 건으로 수집한다.
