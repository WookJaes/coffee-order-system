...
- `src/main/resources/scripts/ranking-process-once.lua`의 Redis Lua는 재구성 잠금 키가 있으면 재시도 가능한 상태를 반환한다. 잠금이 없을 때만 `SET marker NX EX` 성공 시 `ZINCRBY`, 일자 ZSET `EXPIRE`, 일자 `DATA` 완료 상태를 함께 기록한다. `ranking-release-lock.lua`는 UUID 토큰이 일치할 때만 재구성 잠금을 삭제한다. 두 스크립트는 `RedisScript<Long>` Bean으로 시작 시 한 번 로드하며, 마커·ZSET·완료 상태 TTL은 `RANKING_REDIS_KEY_TTL`, 재구성 잠금 TTL은 `RANKING_REDIS_REBUILD_LOCK_TTL`로 설정한다.
- 점수는 `OrderPaidEvent`에 수량 필드가 없고 ADR의 기준이 주문 횟수이므로 이벤트 한 건당 1 증가한다.
