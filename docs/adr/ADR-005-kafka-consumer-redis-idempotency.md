...
- `src/main/resources/scripts/ranking-process-once.lua`의 Redis Lua는 재구성 잠금 키가 있으면 재시도 가능한 상태를 반환한다. 잠금이 없을 때만 `SET marker NX EX` 성공 시 `ZINCRBY`와 일자 ZSET `EXPIRE`를 실행한다. `RedisScript<Long>` Bean으로 시작 시 한 번 로드하며, 마커·ZSET TTL은 `RANKING_REDIS_KEY_TTL`, 재구성 잠금 TTL은 `RANKING_REDIS_REBUILD_LOCK_TTL`로 설정한다.
